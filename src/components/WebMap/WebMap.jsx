...
const WebMap = ({ kpi }) => {
	...
	// This effect runs whenever the selected KPI changes
	useEffect(() => {
		// Check if the new KPI has location data and the map is ready
		if (kpi?.location && mapRef.current) {
			mapRef.current.flyTo({
				center: [kpi.location.longitude, kpi.location.latitude],
				zoom: 14,
				essential: true,
			});
		}
			...
			>
				...
				>
					{baseLayers.map((layer) => (
						<ToggleButton key={layer.value} value={layer.value} aria-label={layer.label} sx={{ width: 80 }}>
							{layer.label}
						</ToggleButton>
					))}
				</ToggleButtonGroup>
